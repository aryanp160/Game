    ...
    pc.onicecandidate = (event) => {
        if (!event.candidate) {
            ...
            if (isHost) {
                ...
                console.log('Host code generated:', code);
            }
        }
    };
...
document.getElementById('connect-btn').onclick = async () => {
    ...
    try {
        ...
        console.log('Joiner attempting to decode code:', code);
        const decodedOffer = atob(code);
        ...
        console.log('Answer code generated:', answerCode);
        prompt('Send this answer code to the host:', answerCode);

        // Host must apply the answer
        if (pc.iceConnectionState !== 'connected') {
            const answerCodeFromHost = prompt('Host: Paste the answer code from the joiner:');
            if (answerCodeFromHost) {
                const decodedAnswer = atob(answerCodeFromHost);
                const answer = JSON.parse(decodedAnswer);
